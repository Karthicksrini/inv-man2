...
let [editing, setediting]= useState(details);
    ...
    useEffect(()=>{  
     ...
     var response=await axios.get("https://inv-man1.herokuapp.com/size/getsize",{
      headers:{
        token:token
      }
    ...
    const editbrand=(async(event,id)=>{
      event.preventDefault();
       await axios.patch(`https://inv-man1.herokuapp.com/size/updatesize/${id}`,
        {
        name:editing.name,
    ...
    const deletebrand=async(id)=>{
      await axios.delete(`https://inv-man1.herokuapp.com/size/deletesize/${id}`,{
        headers:{
          token:token
        }
   ...
   const Addbrand=async(e)=>{
     e.preventDefault();
    await axios.post(`https://inv-man1.herokuapp.com/size/postsize`,
    {
    name:editing.name,
    status:editing.status,
    },{
   ...
   const handleSubmit=(event)=>{
    ...
    return(
        ...
        <div  className="brand1">
            <button onClick={()=>showdiv()} className="addbrand">Add Size</button>
        {console.log(show)}
         <div className={show?"showdiv":"hidediv"}> <br/>
         <form >
  <div className="form-group">
    <label htmlFor="formGroupExampleInput">Sizename</label>
