...
let [editing, setediting]= useState(details);
    ...
    useEffect(()=>{  
     ...
     var response=await axios.get("https://inv-man1.herokuapp.com/group/getgroup",{
      headers:{
        token:token
      }
    ...
    const editbrand=(async(event,id)=>{
      event.preventDefault();
       await axios.patch(`https://inv-man1.herokuapp.com/group/updategroup/${id}`,
        {
        name:editing.name,
    ...
    const deletebrand=async(id)=>{
      await axios.delete(`https://inv-man1.herokuapp.com/group/deletegroup/${id}`,{
        headers:{
          token:token
        }
   ...
   const Addbrand=async(e)=>{
    e.preventDefault();
    await axios.post(`https://inv-man1.herokuapp.com/group/postgroup`,{
     name:editing.name,
     status:editing.status,
    },{
      headers:{
   ...
   const handleSubmit=(event)=>{
    ...
    return(
        ...
        <h2>Manange<span className="dash_title">
            Groups</span></h2>  
            <button onClick={()=>showdiv()} className="addbrand">Add Groups</button>
        {console.log(show)}
         <div className={show?"showdiv":"hidediv"}> <br/>
         <form >
  <div className="form-group">
    <label htmlFor="formGroupExampleInput">Group name</label>
