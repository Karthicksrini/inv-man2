...
let [editing, setediting]= useState(details);
    ...
    useEffect(()=>{  
     ...
     var response=await axios.get(`https://inv-man1.herokuapp.com/brand/getbrand`,{
      headers:{
        token:token
      }
    ...
    const editbrand=(async(event,id)=>{
      event.preventDefault();
       await axios.patch(`https://inv-man1.herokuapp.com/brand/updatebrand/${id}`,{
        name:editing.name,
    ...
    const deletebrand=async(id)=>{
      await axios.delete(`https://inv-man1.herokuapp.com/brand/deletebrand/${id}`,{
        headers:{
          token:token
        }
   ...
   const Addbrand=async(e)=>{
     e.preventDefault();
     await axios.post(`https://inv-man1.herokuapp.com/brand/postbrand`,{
      name:editing.name,
      status:editing.status,
     },{
       headers:{
   ...
   const handleSubmit=(event)=>{
    ...
    return(
        ...
        <h2>Manange<span className="dash_title">
            Brands</span></h2>  
            <button onClick={()=>showdiv()} className="addbrand">Add Brand</button>
        {console.log(show)}
         <div className={show?"showdiv":"hidediv"}> <br/>
         <form >
  <div className="form-group">
    <label htmlFor="formGroupExampleInput">brandname</label>
